...
router.post('/', (req, res) => {
  const { company_id, first_name, last_name, email, phone, mobile_phone, title, address, source } = req.body;
  if (!first_name || !last_name) return res.status(400).json({ error: 'first_name and last_name are required' });
  const result = db.prepare(`INSERT INTO contacts (company_id, first_name, last_name, email, phone, mobile_phone, title, address, source) VALUES (?,?,?,?,?,?,?,?,?)`)
    .run(company_id || null, first_name, last_name, email || null, phone || null, mobile_phone || null, title || null, address || null, source || null);
...
router.patch('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE contacts SET company_id=?, first_name=?, last_name=?, email=?, phone=?, mobile_phone=?, title=?, address=?, source=? WHERE id=?`)
    .run(updates.company_id, updates.first_name, updates.last_name, updates.email, updates.phone, updates.mobile_phone, updates.title, updates.address, updates.source, req.params.id);
