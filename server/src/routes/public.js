...
const {
  ...
  resolveEstimateParty, getContractForEstimate, getJobCustomerType,
} = require('../helpers');
...
router.get('/estimates/:token', (req, res) => {
  ...
  res.json({
    ...
    markup_percent: estimate.markup_percent, markup_amount: estimate.markup_amount, discount_amount: estimate.discount_amount,
...
router.post('/estimates/:token/sign', (req, res) => {
  ...
  const jobId = signEstimateRecord(estimate, { signed_name, signature_data_url });
...
router.get('/invoices/:token', (req, res) => {
  ...
  res.json({
    ...
    markup_percent: invoice.markup_percent, markup_amount: invoice.markup_amount, discount_amount: invoice.discount_amount,
