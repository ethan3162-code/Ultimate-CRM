// Calculator materials only — one half of what used to be one combined "Items & price book"
// page, split out (Sept 2026) so it can be permission-gated separately from the sales-items page
// (see PriceBook.jsx, which — despite the filename — is now labeled "Items"; this file, despite
// ITS filename, is now labeled "Price book"). This is the raw-material catalog that feeds the
// Material Calculator (asphalt, concrete, pavers, border, sand, cement, RCA base), not what gets
// sold to a customer.
...
const BLANK = { name: '', brand: '', description: '', unit: '', unit_price: '', material_key: CALC_MATERIAL_KEYS[0].value, sf_per_pallet: '' };
  ...
  return (
    <>
      <div className="page-head">
        <div>
          <h1>Price book</h1>
          <p className="sub">Priced raw materials that feed the Material Calculator — picking one there fills in its price, and for pavers, the pallet coverage, automatically.</p>
        </div>
        ...
        <div className="card" style={{ marginBottom: 18 }}>
          <form onSubmit={submit} className="form-grid">
            <div className="field">
              ...
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Nicolock Courtstone" required />
            </div>
            <div className="field">
            ...
            {showPalletField(form.material_key) && (
              <>
                <div className="field">
                  <label>Brand</label>
      ...
      {!items ? <div className="loading">Loading…</div> : calcItems.length === 0 ? (
        <div className="card"><div className="empty">No calculator materials yet — add priced materials to auto-fill the Material Calculator.</div></div>
      ) : (
