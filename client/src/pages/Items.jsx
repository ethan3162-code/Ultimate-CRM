...
const BLANK = { description: '', unit: '', unit_price: '', material_key: '' };
...
export default function Items() {
  ...
  function startEdit(item) {
    ...
    setEditDraft({ description: item.description, unit: item.unit || '', unit_price: item.unit_price, material_key: item.material_key || '' });
  }
    ...
    <>
      <div className="page-head">
        <div>
          ...
          <p className="sub">Preset materials and services with their usual cost, so estimates and the material calculator can pull in a description, unit, and price instead of retyping them.</p>
        </div>
        ...
        <div className="card" style={{ marginBottom: 18 }}>
          <form onSubmit={submit} className="form-grid">
            <div className="field" style={{ gridColumn: '1 / -1' }}>
              ...
              <input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="e.g. Asphalt paving, installed" required />
            </div>
            <div className="field">
              <label>Unit</label>
      ...
      {!items ? <div className="loading">Loading…</div> : items.length === 0 ? (
        ...
        <div className="table-wrap">
          <table className="list">
            <thead><tr><th>Description</th><th>Unit</th><th>Unit price</th><th>Linked to</th><th></th></tr></thead>
            <tbody>
              {items.map((item) => {
